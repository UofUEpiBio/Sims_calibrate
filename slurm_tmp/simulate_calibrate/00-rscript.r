...
tcq({
  INDICES <- readRDS("slurm_tmp/simulate_calibrate/INDICES.rds")
})
tcq({
  X <- readRDS(sprintf("slurm_tmp/simulate_calibrate/X_%04d.rds", ARRAY_ID))
})
tcq({
  FUN <- readRDS("slurm_tmp/simulate_calibrate/FUN.rds")
})
tcq({
  mc.cores <- readRDS("slurm_tmp/simulate_calibrate/mc.cores.rds")
})
tcq({
  theta_use <- readRDS("slurm_tmp/simulate_calibrate/theta_use.rds")
})
tcq({
  seeds <- readRDS("slurm_tmp/simulate_calibrate/seeds.rds")
})
...
saveRDS(ans, sprintf("slurm_tmp/simulate_calibrate/03-answer-%03i.rds", ARRAY_ID), compress = TRUE)

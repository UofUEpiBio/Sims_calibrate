...
tcq({
  INDICES <- readRDS("slurm_tmp/abc_calibration/INDICES.rds")
})
tcq({
  X <- readRDS(sprintf("slurm_tmp/abc_calibration/X_%04d.rds", ARRAY_ID))
})
tcq({
  FUN <- readRDS("slurm_tmp/abc_calibration/FUN.rds")
})
tcq({
  mc.cores <- readRDS("slurm_tmp/abc_calibration/mc.cores.rds")
})
tcq({
  simulate_and_calibrate <- readRDS("slurm_tmp/abc_calibration/simulate_and_calibrate.rds")
})
tcq({
  theta_use <- readRDS("slurm_tmp/abc_calibration/theta_use.rds")
})
tcq({
  model_ndays <- readRDS("slurm_tmp/abc_calibration/model_ndays.rds")
})
tcq({
  model_seed <- readRDS("slurm_tmp/abc_calibration/model_seed.rds")
})
tcq({
  simulate_epidemic_observed <- readRDS("slurm_tmp/abc_calibration/simulate_epidemic_observed.rds")
})
tcq({
  simulate_epidemic_calib <- readRDS("slurm_tmp/abc_calibration/simulate_epidemic_calib.rds")
})
tcq({
  summary_fun <- readRDS("slurm_tmp/abc_calibration/summary_fun.rds")
})
tcq({
  proposal_fun <- readRDS("slurm_tmp/abc_calibration/proposal_fun.rds")
})
tcq({
  kernel_fun <- readRDS("slurm_tmp/abc_calibration/kernel_fun.rds")
})
tcq({
  global_n <- readRDS("slurm_tmp/abc_calibration/global_n.rds")
})
tcq({
  run_lfmcmc <- readRDS("slurm_tmp/abc_calibration/run_lfmcmc.rds")
})
tcq({
  seeds <- readRDS("slurm_tmp/abc_calibration/seeds.rds")
})
...
saveRDS(ans, sprintf("slurm_tmp/abc_calibration/03-answer-%03i.rds", ARRAY_ID), compress = TRUE)
